...
interface Cursor {
  ...
  /** HTML elements that retrieve hover effects. */
  targets: Element[]
...
/**
 * Cursor class that handles cursor movement, state, and hover targets.
 */
class Cursor {
  cursor: HTMLDivElement
  ...
  refresh() {
    ...
    this.targets = Array.from(document.querySelectorAll(this.config.target))
  ...
  private init() {
    document.onmousemove = (event) => {
      ...
      // If the cursor is not moved yet, move it to the current position instantly
  ...
  /**
   * Render cursor movement with smooth lerp animation.
   */
  private render() {
    if (this.coords.curr && this.coords.prev) {
      this.coords.prev = lerp_coords(this.coords.prev, this.coords.curr, this.config.lerp_coeff)
  ...
  /**
   * Observe DOM mutations to auto-refresh hover targets.
   */
   ...
   * Ensures hover targets are refreshed on component mount and update.
   */
  setup() {
